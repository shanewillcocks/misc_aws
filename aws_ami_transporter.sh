...
product_name=RHEL8-AMI-Transporter-${datestamp}
dev_aws_id=$(get_octopusvariable "dev_aws_id")
target_acct=$(get_octopusvariable "target_acct")
key_arn=ebscmk_arn

# Get the latest RHEL8 AMI ID
rhel8_latest_ami=$(aws ec2 describe-images --owner ${dev_aws_id} --filters "Name=name,Values=*RHEL8-stable*" --query 'reverse(sort_by(Images, &CreationDate))[0]' | jq -r '.ImageId')
...
rc=$?
if [ ${rc} -eq 0 ]; then
  echo "AMI Transport to Prod started successfully"
else
  echo "AMI Transport to Prod failed to start: ${rc}"
fi
